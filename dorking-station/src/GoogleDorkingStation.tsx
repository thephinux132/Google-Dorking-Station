...
/**
 * Google Dorking Station – Phase 5.5 (Prototype Plan Implementation)
 * -------------------------------------------------------------------
...
const PRESET_PACKS = {
  ...
  "Advanced OSINT": {
    Person: [
      { label: "Resumes (varied)", template: '"{q}" (resume OR cv OR "work history") -jobs -hiring', hint: "Resume/CV variants while excluding job listings." },
      { label: "Academic footprint", template: '"{q}" (site:scholar.google.com OR site:researchgate.net OR site:semanticscholar.org)', hint: "Profiles and publications." },
      { label: "Talks / slides", template: '"{q}" ("talk" OR "presentation" OR "slides" OR keynote) filetype:(pdf OR ppt OR pptx)', hint: "Conference materials and decks." },
      { label: "Public email traces", template: '"{q}" ("@{domain}" OR contact OR email) -site:{domain}', hint: "Mentions of addresses related to a domain." },
    ],
    Place: [
      { label: "City docs (permits/meetings)", template: '"{q}" (permit OR zoning OR agenda OR minutes) site:.gov "{city}" filetype:(pdf OR docx)', hint: "Local government documents by city." },
      { label: "Local reporting", template: '"{q}" site:news.google.com "{city}"', hint: "News coverage for a city." },
      { label: "Wayback snapshots (domain)", template: '{domain}', hint: "Use Wayback in engine menu for snapshots of a domain." },
    ],
    Thing: [
      { label: "Vendor lists / contracts", template: '"{org}" ("RFP" OR "RFQ" OR "contract" OR "SOW") filetype:(pdf OR docx) site:.gov', hint: "Procurement and contract docs (use {org})." },
      { label: "Policies / handbooks", template: '"{org}" (handbook OR policy OR "standard operating procedures") filetype:(pdf OR docx)', hint: "Internal policy documents (public copies)." },
      { label: "Public exports", template: '"{org}" (export OR dump OR backup) filetype:(csv OR xlsx OR json) -site:{domain}', hint: "Data exports mistakenly exposed." },
      { label: "HR lists / rosters", template: '"{org}" (directory OR roster OR "phone list") filetype:(pdf OR xlsx)', hint: "Staff lists and directories." },
      { label: "Tech hardening", template: '"{tech}" (hardening OR benchmark OR CIS OR STIG) filetype:pdf', hint: "Security hardening guides for a tech/product." },
      { label: "Advisories (official)", template: '"{tech}" (site:cisa.gov OR site:nvd.nist.gov)', hint: "Advisory sources for technology/product." },
      { label: "Subdomain crumbs", template: 'site:*.{domain} -www.{domain}', hint: "Search across subdomains for a domain." },
      { label: "Q&A trails", template: '"{tech}" (site:stackoverflow.com OR site:serverfault.com) (error OR troubleshooting)', hint: "Troubleshooting threads for a tech." },
      { label: "Misconfig hints (defanged)", template: '"index of" (backup OR .env OR .git)', hint: "Directory listing indicators (guardrailed)." },
      { label: "Credential keywords (defanged)", template: '"{org}" (password OR api_key OR token) -site:github.com', hint: "Credential terms (guardrailed)." },
    ],
  },
};
...
const ENGINES = [
  ...
  // External research engines (best used with simple tokens like domains)
  { key: 'crt', label: 'crt.sh', base: 'https://crt.sh/', param: 'q' },
  { key: 'wayback', label: 'Wayback', tmpl: 'https://web.archive.org/web/*/{q}/*' },
  { key: 'publicwww', label: 'PublicWWW', tmpl: 'https://publicwww.com/websites/%22{q}%22/' },
];
...
function toSearchUrl(engineKey: string, query: string) {
  const e = getEngine(engineKey) as any;
  if (e.tmpl) {
    return String(e.tmpl).split('{q}').join(encodeURIComponent(query));
  }
  const u = new URL(e.base);
  if (e.param) u.searchParams.set(e.param, query);
  return u.toString();
}
...
type BuilderMode = 'simple' | 'group';
type GroupNode = { id: string; type: 'GROUP'; op: 'AND' | 'OR'; children: (GroupNode | GroupChip)[] };
type GroupChip = { id: string; type: 'CHIP'; chipType: ChipType; value: string };
...
export default function GoogleDorkingStation() {
  ...
  const [communityPacks, setCommunityPacks] = useLocalJSON<any[]>(COMMUNITY_PACKS_KEY, []); // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const [subs, setSubs] = useState({ q: '', date: '', location: '', filetype: '', org: '', domain: '', tech: '', city: '', country: '', year: '' });
  ...
  const [groupRoot, setGroupRoot] = useState<GroupNode>({ id: 'g_root', type: 'GROUP', op: 'AND', children: [{ id: 'gc0', type: 'CHIP', chipType: 'PHRASE', value: '' }] });
  ...
  function chipToText(t: ChipType, v: string) {
    const val = String(v || '').trim();
    if (t === 'PHRASE') return '"' + (val || '{q}') + '"';
    if (t === 'OR') {
      if (!val) return '';
      const terms = val.split(',').map((s) => s.trim()).filter(Boolean);
      return terms.length ? '(' + terms.join(' OR ') + ')' : '';
    }
    if (t === 'SITE' && val) return 'site:' + val;
    if (t === 'FILETYPE' && val) return 'filetype:' + val;
    if (t === 'INURL' && val) return 'inurl:' + val;
    if (t === 'INTITLE' && val) return 'intitle:' + val;
    if (t === 'EXCLUDE' && val) {
      const ex = val.split(',').map((s) => s.trim()).filter(Boolean).map((w) => (w.charAt(0) === '-' ? w : '-' + w));
      return ex.join(' ');
    }
    return '';
  }

  function renderGroup(n: GroupNode | GroupChip): string {
    if ((n as GroupChip).type === 'CHIP') {
      const c = n as GroupChip;
      return chipToText(c.chipType, c.value);
    }
    const g = n as GroupNode;
    const rendered = g.children.map(renderGroup).filter(Boolean);
    if (rendered.length === 0) return '';
    const sep = g.op === 'AND' ? ' ' : ' OR ';
    const s = rendered.join(sep).trim();
    return g.op === 'AND' ? s : '(' + s + ')';
  }

  const builtTemplate = useMemo(() => {
    if (builderMode === 'group') {
      return renderGroup(groupRoot).replace(/\s+/g, ' ').trim();
    }
    const parts: string[] = [];
    for (let i = 0; i < chips.length; i++) {
      const c = chips[i];
      const t = chipToText(c.type, c.value);
      if (t) parts.push(t);
    }
    return parts.join(' ').replace(/\s+/g, ' ').trim();
  }, [builderMode, groupRoot, chips]);
    ...
    <div className="min-h-screen w-full bg-gray-50 text-gray-900 dark:bg-gray-900 dark:text-gray-100 p-6">
      <div className="mx-auto max-w-6xl">
        <header className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Google Dorking Station</h1>
                    ...
                    <div className="flex items-start gap-2 shrink-0">
                      ...
                      <a
                        ...
                        className={'rounded-2xl px-3 py-2 text-sm border shadow-sm ' + (
                          disabled
                            ? 'cursor-not-allowed pointer-events-none bg-gray-200 text-gray-700 border-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600'
                            : 'bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700'
                        )}
                        href={disabled ? undefined : toSearchUrl(engine, query)}
                        target="_blank"
                        rel="noreferrer"
                        title={disabled ? 'Enter a keyword above first.' : 'Open on ' + engineLabel}
                      >
                        {'Open in ' + engineLabel}
                      </a>
          ...
          <section className="mt-8 grid lg:grid-cols-[2fr,1fr] gap-6">
            <div className="bg-white dark:bg-gray-800 rounded-2xl border dark:border-gray-700 shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold">Dork Builder</h3>
                <div className="flex items-center gap-2">
                  <button onClick={() => setBuilderMode('simple')} className={'text-xs rounded-xl px-3 py-1.5 border ' + (builderMode==='simple' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-800 dark:bg-gray-800 dark:text-gray-100 border-gray-300 dark:border-gray-600')}>Simple</button>
                  <button onClick={() => setBuilderMode('group')} className={'text-xs rounded-xl px-3 py-1.5 border ' + (builderMode==='group' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-800 dark:bg-gray-800 dark:text-gray-100 border-gray-300 dark:border-gray-600')}>Group</button>
                </div>
              </div>
              {builderMode === 'simple' ? (
              <>
              ...
              <div className="rounded-xl border dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 p-3 min-h-[88px]">
                ...
                <div className="rounded-xl border dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 p-3">
                  {(() => {
                    const addChild = (parentId: string, child: GroupNode | GroupChip) => {
                      setGroupRoot(prev => {
                        const clone = structuredClone(prev) as GroupNode;
                        function visit(n: GroupNode) {
                          if (n.id === parentId) { n.children.push(child); return true; }
                          for (const ch of n.children) if ((ch as any).type === 'GROUP' && visit(ch as GroupNode)) return true;
                          return false;
                        }
                        visit(clone);
                        return clone;
                    ...
                    const updateChipNode = (id: string, patch: Partial<GroupChip>) => {
                      setGroupRoot(prev => {
                        const clone = structuredClone(prev) as GroupNode;
                        function visit(n: GroupNode) {
                          for (let i=0;i<n.children.length;i++) {
                            const ch = n.children[i] as any;
                            if (ch.type === 'CHIP' && ch.id === id) { n.children[i] = { ...ch, ...patch }; return true; }
                            if (ch.type === 'GROUP' && visit(ch)) return true;
                          }
                          return false;
                        }
                        visit(clone);
                    ...
                    const updateGroupNode = (id: string, patch: Partial<GroupNode>) => {
                      setGroupRoot(prev => {
                        const clone = structuredClone(prev) as GroupNode;
                        function visit(n: GroupNode): boolean {
                          if (n.id === id) { (n as any).op = patch.op || n.op; return true; }
                          for (const ch of n.children) if ((ch as any).type === 'GROUP' && visit(ch as GroupNode)) return true;
                          return false;
                        }
                        visit(clone);
                    ...
                    const removeNode = (id: string) => {
                      ...
                      setGroupRoot(prev => {
                        const clone = structuredClone(prev) as GroupNode;
                        function visit(n: GroupNode) {
                          n.children = n.children.filter((ch: any) => ch.id !== id);
                          for (const ch of n.children) if ((ch as any).type === 'GROUP') visit(ch as GroupNode);
                        }
                        visit(clone);
                        return clone;
                      });
                    };
                    const renderEditor = (node: GroupNode | GroupChip, isRoot: boolean) => {
                      if ((node as any).type === 'CHIP') {
                        const c = node as GroupChip;
                        return (
                          <div key={c.id} className="bg-white dark:bg-gray-800 border dark:border-gray-600 rounded-2xl px-3 py-2 shadow-sm flex items-center gap-2">
                            <select value={c.chipType} onChange={(e) => updateChipNode(c.id, { chipType: e.target.value as ChipType })} className="text-xs rounded-lg border dark:border-gray-600 dark:bg-gray-700 px-2 py-1">
                              {(['PHRASE','OR','SITE','FILETYPE','INURL','INTITLE','EXCLUDE'] as ChipType[]).map(t => <option key={t} value={t}>{t}</option>)}
                            </select>
                            <input className="text-xs border dark:border-gray-600 dark:bg-gray-700 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder={CHIP_HELP[c.chipType]} value={c.value} onChange={(e) => updateChipNode(c.id, { value: e.target.value })} style={{ minWidth: 140 }} />
                            {!isRoot && <button className="text-gray-500 hover:text-red-600" onClick={() => removeNode(c.id)} title="Remove"><XIcon/></button>}
                          </div>
                        );
                      }
                      const g = node as GroupNode;
                      return (
                        <div key={g.id} className="rounded-xl border dark:border-gray-600 bg-white/50 dark:bg-gray-800/50 p-3">
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-xs">Group</span>
                            <select value={g.op} onChange={(e) => updateGroupNode(g.id, { op: e.target.value as 'AND' | 'OR' })} className="text-xs rounded-lg border dark:border-gray-600 dark:bg-gray-700 px-2 py-1">
                              <option value="AND">AND</option>
                              <option value="OR">OR</option>
                            </select>
                            {!isRoot && <button className="text-gray-500 hover:text-red-600" onClick={() => removeNode(g.id)} title="Remove group"><XIcon/></button>}
                            <div className="ml-auto flex gap-2">
                              <button onClick={() => addChild(g.id, { id: 'gc' + Math.random().toString(36).slice(2,6), type:'CHIP', chipType:'PHRASE', value:'' })} className="text-xs rounded-lg px-2 py-1 border bg-white hover:bg-gray-100 dark:bg-gray-700 dark:border-gray-600 dark:hover:bg-gray-600">+ Chip</button>
                              <button onClick={() => addChild(g.id, { id: 'gg' + Math.random().toString(36).slice(2,6), type:'GROUP', op:'AND', children: [] })} className="text-xs rounded-lg px-2 py-1 border bg-white hover:bg-gray-100 dark:bg-gray-700 dark:border-gray-600 dark:hover:bg-gray-600">+ Group</button>
                            </div>
                          </div>
                          <div className="space-y-2">
                            {g.children.map((ch) => renderEditor(ch, false))}
                          </div>
                        </div>
                      );
                    };
                    return renderEditor(groupRoot, true);
                  })()}
                </div>
              )}
              <div className="mt-4">
                ...
                <div className="grid md:grid-cols-4 gap-2">
                  {['q','date','location','filetype','org','domain','tech','city','country','year'].map(k => (
                    <div key={k}>
              ...
              <div className="mt-4">
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <a
                    className={'rounded-2xl px-3 py-2 text-sm border shadow-sm ' + (
                      (!renderedQuery || disabled)
                        ? 'cursor-not-allowed pointer-events-none bg-gray-200 text-gray-700 border-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600'
                        : 'bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700'
                    )}
                    href={!renderedQuery || disabled ? undefined : toSearchUrl(engine, renderedQuery)}
                    target="_blank"
                    rel="noreferrer"
                    title={!renderedQuery || disabled ? 'Enter a keyword above first.' : 'Open on ' + getEngineLabel(engine)}
                  >
                    {'Open in ' + getEngineLabel(engine)}
                  </a>
                </div>
              </div>
            </div>
